...
static void Task_NewGameBirchSpeech_WaitForInitialFade(u8);
...
static void Task_NewGameBirchSpeech_Init(u8 taskId)
{
    ...
    gTasks[taskId].func = Task_NewGameBirchSpeech_WaitForInitialFade;
    gTasks[taskId].tPlayerSpriteId = spriteId;
    gTasks[taskId].tPlayerGender = MALE;
    gTasks[taskId].data[3] = 0xFF;
    gTasks[taskId].tTimer = 0;
    PlayBGM(MUS_RG_OAK_LAB);
    SetGpuReg(REG_OFFSET_BG1HOFS, gTasks[taskId].tBG1HOFS);
    ShowBg(0);
    ShowBg(1);
}

static void Task_NewGameBirchSpeech_WaitForInitialFade(u8 taskId)
{
    if (!gPaletteFade.active)
    {
...
static void AddBirchSpeechObjects(u8 taskId)
{
    ...
    // Use a generic scientist sprite until Elm-specific intro art is ported.
    birchSpriteId = CreateObjectGraphicsSprite(OBJ_EVENT_GFX_SCIENTIST_1, SpriteCB_Null, 0x88, 0x3C, 1);
